...
class TvRecommendedView extends StatefulWidget {
  const TvRecommendedView({Key? key, required this.id}) : super(key: key);

  final int id;
  @override
  State<TvRecommendedView> createState() => _TvRecommendedViewState();
}

class _TvRecommendedViewState extends State<TvRecommendedView> {

  Future<TrendingTV> getRecommendMovies(int id) async {
    String jsonUrl = 'https://api.themoviedb.org/3/tv/' + id.toString() + '/recommendations?api_key=53b6f70df0427cdf36b1c1679950f19b&language=en-US&page=1';

    var response = await get(Uri.parse(jsonUrl));
    var json = jsonDecode(response.body);
    TrendingTV tv = TrendingTV.fromJson(json);
    return tv;
  }
          ...
          if(snapshot.connectionState == ConnectionState.done) {
            if(snapshot.hasData) {
              if(snapshot.data?.results.length == 0) {
                return ErrorNotFoundWidget(errorType: '\"Recommended TV Shows\"');
              } else {
